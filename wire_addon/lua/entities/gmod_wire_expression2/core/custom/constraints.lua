...
local function Weldit(self, ent1, ent2, nc, fl)
	if ent1:IsValid() and ent2:IsValid() and type(ent1)!="Player" and type(ent2)!="Player" then
...
e2function void entity:ropeTo(entity ent2,vector localposition1,vector localposition2,number addlength,number forcelimit,number width,string material,number rigid)
	...
	if this:IsValid() and ent2:IsValid() and type(this)!="Player" and type(ent2)!="Player" then
...
e2function void entity:setPhysProp(string material,number gravity)
	...
	if this:IsPlayer() or !this:IsValid() or !isOwner(self,this) then return end
	construct.SetPhysProp( ply, this, 0, nil,  { GravityToggle = tobool(gravity), Material = material } )
...
e2function void entity:removeAllConstraints()
	if isOwner(self,this) and this:IsValid() then
...
e2function void entity:noCollide(entity ent)
	...
	if isOwner(self,this) and this:IsValid() and this!=ent then
...
e2function table entity:getConstraintTable()
	...
	if this:IsPlayer() or !this:IsValid() or !isOwner(self,this) then return DEFAULT end
...
local function AdvBallsocketIt(self, ent1, ent2, LPos1, LPos2, forcelimit, torquelimit, xmin, ymin, zmin, xmax, ymax, zmax, xfric, yfric, zfric, onlyrotation, nocollide )
    if ent1:IsValid() and ent2:IsValid() and type(ent1)!="Player" and type(ent2)!="Player" then
        if isOwner(self, ent1) and isOwner(self, ent2) and ent1!=ent2 then
            local AdvBallsocket = constraint.AdvBallsocket(ent1, ent2, 0, 0, LPos1, LPos2, forcelimit, torquelimit, xmin, ymin, zmin, xmax, ymax, zmax, xfric, yfric, zfric, onlyrotation, nocollide)
                undo.Create("advballsocket")
                undo.AddEntity(AdvBallsocket)
                undo.SetPlayer( self.player )
                undo.Finish()
                self.player:AddCleanup( "constraints", AdvBallsocket )
        else return end
    else return end
end


e2function void entity:advballsocketTo(entity ent, vector localposition1, vector localposition2, vector minvec, vector maxvec, vector fricvec, number forcelimit, number torquelimit, number onlyrotation, number nocollide)
    local ply = self.player
    if GetConVar("sbox_e2_constraints"):GetInt() == 1 then
        if not ply:IsAdmin() then return end
    end
    ...
    local ymax = maxvec[2]
    local zmax = maxvec[3]
    local xfric = fricvec[1]
    local yfric = fricvec[2]
    local zfric = fricvec[3]
    AdvBallsocketIt(self, this, ent, lpos1, lpos2, forcelimit, torquelimit, xmin, ymin, zmin, xmax, ymax, zmax, xfric, yfric, zfric, onlyrotation, nocollide)
end
